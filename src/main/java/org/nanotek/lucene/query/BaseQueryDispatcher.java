	...
	public void dispatch(QueryObject<?> queryObject , final QueryContext queryContext) {
		...
		try {
			...
			if (indexSearcher !=null)
			{
				...
				//TODO: Change Query Parser Analyzer to Analyzer used during Indexing.
				if (queryParser == null)
					queryParser = new QueryParser(Version.LUCENE_46, (String) targetField.getId(), new StandardAnalyzer(Version.LUCENE_46));
...
//		return results;
	...
	private ScoreDocBase defineLastScoreDoc(final QueryContext queryContext, final TopDocs topDocs) {
